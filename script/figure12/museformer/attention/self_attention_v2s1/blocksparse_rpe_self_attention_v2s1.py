...
class BlocksparseRpeSelfAttentionV2S1(RpeSelfAttentionV2S1):
    ...
    def do_qk_scores_for_sx(self, base_sum_q, base_sum_k, base_reg_k, bsz, sum_len, reg_len, attn_mask=None):
        ...
        a, b = do_qk_scores_for_part(self, tgt, src, bsz, sum_len, sum_len + reg_len, attn_mask, 'sx')
        self.convert_overhead += b
        return a

    def add_rpe_for_sx(self, attn_scores_inc_sx, rel_sum_qs, r_list, rel_indices, bsz, sum_len, attn_mask=None):
        return add_rpe_for_part(self, attn_scores_inc_sx, rel_sum_qs, r_list, rel_indices,
                                bsz, sum_len, attn_mask, 'sx')
    ...
    def do_attn_softmax_for_sx(self, attn_scores_inc_sr, attn_mask=None):
        a, b = do_attn_softmax_for_part(self, attn_scores_inc_sr, attn_mask, 'sx')
        self.convert_overhead += b
        return a
    ...
    def do_av_mul_for_sx(self, attn_weights_inc_sr, base_sum_v, base_reg_v, attn_mask=None, tgt_len=None):
        ...
        a, b = do_av_mul_for_part(self, attn_weights_inc_sr, v, attn_mask, 'sx', tgt_len)
        self.convert_overhead += b
        return a
    ...
    def do_qk_scores_for_rx(
        ...
        a, b = do_qk_scores_for_part(self, reg_q, k, bsz, reg_len, sum_len + reg_len, attn_mask, 'rx')
        self.convert_overhead += b
        return a

    def add_rpe_for_rx(self, attn_scores_inc, rel_reg_qs, r_list, rel_indices,
                       bsz, reg_len, attn_mask=None):
        return add_rpe_for_part(self, attn_scores_inc, rel_reg_qs, r_list, rel_indices,
                                bsz, reg_len, attn_mask, 'rx')

    def do_masking_for_rx(self, attn_scores_inc, attn_mask):
        return attn_scores_inc

    def do_attn_softmax_for_rx(self, attn_scores_inc, attn_mask=None):
        a, b = do_attn_softmax_for_part(self, attn_scores_inc, attn_mask, 'rx')
        self.convert_overhead += b
        return a
    ...
    def do_av_mul_for_rx(self, attn_weights_inc, base_sum_v2, base_reg_v, attn_mask=None, tgt_len=None):
        ...
        a, b = do_av_mul_for_part(self, attn_weights_inc, v, attn_mask, 'rx', tgt_len)
        self.convert_overhead += b
        return a
