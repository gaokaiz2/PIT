        ...
        *args, **kwargs,
        ...
        if sum_len > 0:
            ...
            if sx_label in self.instant_pocket:
                sx_attn_mask = self.instant_pocket[sx_label]
            else:
            #     assert sx_attn_mask is not None
            #     sx_spa = DynamicSparseAttention((~sx_attn_mask).int())
                self.instant_pocket[sx_label] = sx_attn_mask
            ...
            query_layer = temp_q.permute(1, 2, 0, 3)
            key_layer = temp_k.permute(1, 2, 0, 3)
            value_layer = temp_v.permute(1, 2, 0, 3)
            attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))

            attention_scores = attention_scores / math.sqrt(self.head_dim)
            attention_mask = (~sx_attn_mask).int()
            ...
            attention_probs = nn.functional.softmax(attention_scores, dim=-1).to(value_layer.dtype)
            # print(attention_probs.dtype, value_layer.dtype, temp_q.dtype)

            sum_x2 = torch.matmul(attention_probs, value_layer).to(temp_q.dtype)
            # sum_x2 = sx_spa(
            ...
            # print(sum_x2.shape, query_layer.shape, key_layer.shape, value_layer.shape)
            ...
            del sx_label, temp_q, temp_k, temp_v, sx_attn_mask
        ...
        if rx_label in self.instant_pocket:
            rx_attn_mask = self.instant_pocket[rx_label]
        else:
        #     assert rx_attn_mask is not None
        #     rx_spa = DynamicSparseAttention((~rx_attn_mask).int())
            self.instant_pocket[rx_label] = rx_attn_mask
        ...
        attention_scores = attention_scores / math.sqrt(self.head_dim)
        attention_mask = (~rx_attn_mask).int()
        # extended_attention_mask = attention_mask[:, None, :, :]
        extended_attention_mask = (1.0 - attention_mask) * -10000.0
        attention_scores = attention_scores + extended_attention_mask
        ...
        reg_output = torch.matmul(attention_probs, value_layer).to(temp_q.dtype)
        ...
        del temp_q, temp_k, temp_v, rx_label, rx_attn_mask
