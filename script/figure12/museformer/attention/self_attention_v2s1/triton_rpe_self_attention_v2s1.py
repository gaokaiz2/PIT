...
class TritonRpeSelfAttentionV2S1(RpeSelfAttentionV2S1):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        assert self.num_relation_types == 0, 'sparta does not support rpe'
        self.triton = TritonDynamicAttention(32, 32, self.num_heads, profile=True)
    ...
    def forward(
        ...
        *args, **kwargs,
        ...
        if sum_len > 0:
            ...
            # print(temp_q.shape, sx_spa.shape, sx_spa.unsqueeze(0).repeat(self.num_heads, 1, 1).shape)
            sum_x2 = self.triton(
                temp_q.permute(1, 2, 0, 3).float(),
                temp_k.permute(1, 2, 0, 3).float(),
                temp_v.permute(1, 2, 0, 3).float(),
                sx_spa.unsqueeze(0).repeat(self.num_heads, 1, 1),
        ...
        # assert not temp_k.isnan().any()
        # assert not temp_v.isnan().any()
        # print(temp_q.shape, rx_spa.shape, rx_spa.unsqueeze(0).repeat(self.num_heads, 1, 1).shape)
        reg_output = self.triton(
            temp_q.permute(1, 2, 0, 3).float(),
            temp_k.permute(1, 2, 0, 3).float(),
            temp_v.permute(1, 2, 0, 3).float(),
            rx_spa.unsqueeze(0).repeat(self.num_heads, 1, 1),
