...
class RpeSelfAttentionV2S1(nn.Module):
    ...
    def forward(
        ...
        *args, **kwargs,
        ...
        if sum_len > 0:
            ...
            # print(base_sum_q.shape, base_sum_k.shape, base_reg_k.shape)
            ...
            attn_scores_inc_sx = self.add_rpe_for_sx(attn_scores_inc_sx, rel_sum_qs, r_list, rel_indices,
                                                     bsz, sum_len, attn_mask=attn_mask)

            # if self.layer_idx == 3:
            #     with open('attn_mask.bin', 'wb') as f:
            #         torch.save(attn_mask, f)
            #         print('saved attn_mask')
            ...
            # if self.layer_idx == 3:
            #     with open('attn_weights_inc_sx.bin', 'wb') as f:
            #         torch.save(attn_weights_inc_sx, f)
            #         print('saved attn_weights_inc_sx')
        ...
        attn_scores_inc_rx = self.add_rpe_for_rx(attn_scores_inc_rx, rel_reg_qs, r_list, rel_indices,
    ...
    def add_rpe_for_sx(self, attn_scores_for_sum, rel_sum_qs, r_list, rel_indices, bsz, sum_len, **kwargs):
        raise NotImplementedError
